...
CREATE TABLE IF NOT EXISTS applications (
    id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    player_id BIGINT NOT NULL REFERENCES players(id) ON DELETE CASCADE,
    company_tier VARCHAR(20) NOT NULL CHECK (company_tier IN ('startup', 'mid_tier', 'big_tech')),
    company_id BIGINT REFERENCES companies(id) ON DELETE SET NULL,
    market_state_id BIGINT REFERENCES market_states(id) ON DELETE SET NULL,
    market_percent NUMERIC(5,2) NOT NULL DEFAULT 0 CHECK (market_percent >= 0 AND market_percent <= 100),
    market_multiplier NUMERIC(6,3) NOT NULL DEFAULT 1.000,
    message TEXT NOT NULL,
    score_snapshot INTEGER NOT NULL DEFAULT 0 CHECK (score_snapshot >= 0),
    resume_multiplier NUMERIC(6,3) NOT NULL DEFAULT 1.000,
